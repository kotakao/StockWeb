...
> 完成一區後在標題打勾並記錄 commit hash。
> 需 StockDCbot 端配合的前置派工（DC-x 編號）一律收錄於 StockDCBot repo 的
> todolist.md（資料源與 schema 歸 Python 端管，見 AGENT.md 鐵律）；本檔 W 區塊
> 僅以編號引用前置、不收錄 DC 區塊，避免跨 repo 絕對路徑與派工歸屬不明。
...
| 1.1 ✅ | W5 公司總覽頁、W-R1 方案分層重構、W6 公司資訊與法說會、W7 持股狀態（前置 DC-J/K/L 皆完成） | 完成 |
...
```text
你的工作目錄是 StockWeb 專案根目錄（本 repo，Windows、git repo、main 分支）。
請先閱讀 AGENT.md（鐵律與工程慣例）與姊妹專案 StockDCBot 的
docs/stockweb-spec.md（StockDCBot 本機路徑見 AGENT.md 專案定位段）。
...
## ☑ W-R1：方案分層重構（單專案 → 四專案，v2 修訂版）— commit `4f8eac2`
...
8. 同步更新文件：docs/architecture.md 的資料夾對照表、§1 總覽圖與 §4 追蹤
   ...
   姊妹專案 StockDCBot 的 docs/stockweb-spec.md §5 的方案結構
   （spec 在另一個 repo，該檔更新後請於回報中註明，由管理流程在
   StockDCbot 端 commit，你不要在該 repo 執行 git 操作）。

驗收：dotnet build 無警告無錯誤；禁止進行dotnet test，未來會再進行test；dotnet run --project
...
## ☑ W6：公司近期資訊（新聞）與法說會 — commit `08ab3b3`

```text
你的工作目錄是 StockWeb 專案根目錄（本 repo，Windows、git repo、main 分支）。
請先閱讀 AGENT.md（鐵律與工程慣例）與姊妹專案 StockDCBot 的
docs/stockweb-spec.md（StockDCBot 本機路徑見 AGENT.md 專案定位段）。
前置：DC-K 已完成（investor_conferences 表存在，內容為「主旨含法人說明會」
的重大訊息原始列，詳見 StockDCBot todolist 的 DC-K）；W5 已完成（總覽頁佈局）。
...
4. 法說會區塊：讀 investor_conferences（唯讀；DC-K 入庫的是重大訊息原始列，
   實際欄位以 DC-K 完成後的 README 記載為準），顯示該公司近期與未來場次
   （事實發生日＝召開日、主旨、說明全文的關鍵欄摘要——說明欄為結構化
   文字，含召開時間/地點，解析放 Services 純類別以利測試、解析失敗時
   顯示原文不中斷）；另在 /calendar 行事曆頁把未來 30 日全市場法說會
   （依事實發生日）與除權息事件並列（不同顏色標記）。
...
## ☑ W7：觀察名單持股狀態板 — commit `c229542`

```text
你的工作目錄是 StockWeb 專案根目錄（本 repo，Windows、git repo、main 分支）。
請先閱讀 AGENT.md（鐵律與工程慣例）與姊妹專案 StockDCBot 的
docs/stockweb-spec.md（StockDCBot 本機路徑見 AGENT.md 專案定位段）。
前置：DC-L 已完成（holdings 表存在，見 StockDCBot todolist）。完成本區後，AGENT.md 鐵律第 2 條的
...
## ☐ W9：產業與同業比較（前置：StockDCBot todolist 的 DC-M 產業分類資料源）

```text
前置：DC-M 已完成（company_profile 表存在，見 StockDCBot todolist，屆時細化）。
...
- ✅ W-R1 方案分層重構（單專案 → 四專案，Web → Data → Services → Models）— commit `4f8eac2`
- ✅ DC-K 法說會（重大訊息）資料源（StockDCbot 端，W6 前置）— StockDCBot commit `a31b92c`
- ✅ DC-L 持股資料表（StockDCbot 端，W7 前置）— StockDCBot commit `90ff92f`
- ✅ 功能區 W6 公司近期資訊（新聞）與法說會 — commit `08ab3b3`
- ✅ 功能區 W7 觀察名單持股狀態板 — commit `c229542`
